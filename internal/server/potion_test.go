...
func TestPotionEffectsApplyExpectedDurationsAndTurtleMaster(t *testing.T) {
	tests := []potionEffectDurationTest{
		{name: "regeneration", potion: game.PotionRegeneration, effects: []game.MobEffectInstance{game.NewMobEffectInstance(game.MobEffectRegeneration, 899, 0, false, true, true)}},
		{name: "poison", potion: game.PotionLongPoison, effects: []game.MobEffectInstance{game.NewMobEffectInstance(game.MobEffectPoison, 1799, 0, false, true, true)}},
		{name: "turtle master", potion: game.PotionTurtleMaster, effects: []game.MobEffectInstance{
			game.NewMobEffectInstance(game.MobEffectSlowness, 399, 3, false, true, true),
			game.NewMobEffectInstance(game.MobEffectResistance, 399, 2, false, true, true),
		}},
	}
...
func TestPotionWaterBreathingRefillsAirExpiresAndFireResistanceBlocksLava(t *testing.T) {
	...
	player := actor.snapshotPlayer()
	if player.AirSupply != 14 {
		t.Fatalf("water breathing air refill = %d, want 14", player.AirSupply)
	}
	...
	if actor.snapshotPlayer().AirSupply != 17 {
		t.Fatalf("air after water breathing expiry = %d, want 17", actor.snapshotPlayer().AirSupply)
	}
	...
	health := actor.snapshotPlayer().Health

	runtime.Tick()

	if actor.snapshotPlayer().Health != health {
		t.Fatalf("fire resistance did not block lava damage: health %v -> %v", health, actor.snapshotPlayer().Health)
	}
}
...
func TestPotionCustomEffectsScaleChainsMetadataAndInventory(t *testing.T) {
	...
	assertPlayerEffect(t, player, game.MobEffectNightVision, 1799, 0)
	assertPlayerEffect(t, player, game.MobEffectSpeed, 9, 0)
	...
	assertPlayerEffect(t, player, game.MobEffectSpeed, 8, 0)
