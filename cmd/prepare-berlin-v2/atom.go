...
func discoverAtomResources(ctx context.Context, source string) ([]remoteResource, error) {
	if supportedLoD2Resource(source) {
		return []remoteResource{{URL: source}}, nil
	}
	...
	for len(queue) > 0 {
		...
		for _, link := range links {
			...
			if supportedLoD2Resource(resolved) {
	...
	result := make([]remoteResource, 0, len(locations))
	for _, location := range locations {
		result = append(result, remoteResource{URL: location})
	}
...
func supportedLoD2Resource(location string) bool {
